...
const content = {
  ...
  "pageContent": [
        ...
        <template v-slot:prepend>
          <v-tabs color="success" v-model="dataType" grow>
        ...
        <v-list-item-group v-else v-model="currentItemIndex" mandatory dense color="success" @change="doUiAction('changeCurrentItemIndex')">
          <v-list-item
      ...
      <div class="jh-page-second-bar">
        <v-row class="align-center">
          <v-col cols="12" xs="12" sm="12" md="4" xl="3">
            <div class="py-4 text-body-1 font-weight-bold">组织、角色
              <!-- 帮助页按钮 -->
      ...
      <div class="jh-page-body-container">
        ...
        <v-card class="rounded-lg">
          ...
          <v-row class="ma-0 py-0">
            <v-col :style="{maxWidth: 'calc(100% - ' + (isMobile ? 0 : 270) + 'px)'}" class="ma-0 pa-0">
              ...
              <v-data-table
                ...
                <template v-slot:item.roleId="{ item }">
                  {{ item.roleId | getRoleName(roleListFromBackend) }}
                </template>
                <template v-slot:item.groupId="{ item }">
                  {{ item.groupId | getGroupName(groupListFromBackend) }}
                </template>
                <template v-slot:item.userId="{ item }">
                  {{ item.userId | getUserName(userListFromBackend) }}
                </template>
                <!-- 表格操作按钮 -->
            ...
            <!-- 右侧组织、角色信息 >>>>>>>>>>>>> -->
            <v-col style="max-width: 270px; border-left: 1px solid #eee;" class="px-6 ml-3" v-if="!isMobile">
              ...
              <v-form ref="formCurrentDataTypeDetail" lazy-validation>
                ...
                <v-list-item v-for="item of dataTypeFieldList" class="pl-0 pt-4 pr-0">
                  <v-autocomplete
                    ...
                    :rules="item.require == false ? validationRules.nullRules : validationRules.requireRules" :items="constantObj[item.value]"></v-autocomplete>
                  <v-textarea
                    :disabled="item.disabled"
  ...
  "common": {
    filters: {
      getGroupName(groupId, groupList) {
        return (groupList.find(({value}) => value === groupId) || {}).text || groupId;
      },
      getRoleName(roleId, roleList) {
        return (roleList.find(({value}) => value === roleId) || {}).text || roleId;
      },
      getUserName(userId, userList) {
        return (userList.find(({value}) => value === userId) || {}).text || userId;
      },
    },
    data: {
      ...
      groupKeys: [
        {text: "组织Id", value: "groupId", disabled: true},
      ...
      roleKeys: [
        {text: "角色ID", value: "roleId", disabled: true},
    ...
    computed: {
      dataTypeFieldList() {
        return this.dataType === 0 ? this.groupKeys : this.roleKeys;
      },
      dataTypeName() {
        return ['组织', '角色'][this.dataType];
      },
      currentItemList() {
        return this.dataType === 0 ? this.groupListFromBackend : this.roleListFromBackend;
      },
      dataTypeData() {
        let searchKey;
        if (this.dataType === 0) {
        ...
        if (this.tabsSearchKeyword) {
          return this.currentItemList.filter((funObj) => funObj.data[searchKey[0]].includes(this.tabsSearchKeyword) || funObj.data[searchKey[1]].includes(this.tabsSearchKeyword));
        } else {
          return this.currentItemList;
        }
      },
      currentDataTypeItem() {
        if (this.currentItemList && this.currentItemList[this.currentItemIndex]) {
          return this.currentItemList[this.currentItemIndex].data;
        }
        return {};
      },
      relationDataList() {
        ...
        return this.relationDataListFromBackend.filter((item) => {
    ...
    methods: {
      async doUiAction(uiActionId, uiActionData) {
        switch (uiActionId) {
          ...
          case 'createRelationDataItem':
            ...
            await this.getUserGroupRoleList(uiActionData);
        ...
        const result = await window.jianghuAxios({
          data: {
            appData: {
              pageId: 'userGroupRoleEnterprise',
              actionId: 'selectUser',
              where: {appId: window.appInfo.appId}
            }
          }
        });
      ...
      async prepareCreateRelationDataForm() {
        ...
        // 预设 groupId 或 roleId
        if (this.dataType === 0) {
          this.createRelationDataFormData.groupId = this.dataTypeData[this.currentItemIndex].value;
          this.createRelationDataFormData.roleId = 'default';
        }
        if (this.dataType === 1) {
          // this.createRelationDataFormData.groupId = this.groupListFromBackend[0].value;
          this.createRelationDataFormData.roleId = this.dataTypeData[this.currentItemIndex].value;
        }
      },
      ...
      // description: ✅关闭添加组织、角色的表单弹窗
      async closeFormDialog() {
        this.isCreateUserDialogShow = false;
        this.isCreateGroupDialogShown = false;
        this.isCreateRoleDialogShown = false;
      },
      // description: ✅刷新组织、角色的数据
