/* eslint-disable */
const content = {
  pageType: "<=$ pageType $=>", pageId: "<=$ pageId $=>", table: "<=$ table $=>", pageName: "<=$ pageName $=>", template: "jhTemplateV4", version: 'v3', 
  resourceList: [
    {
        ...
        { 
          ...
          formItemList: [
            ...
            */
            ...
            //===//<=%- elif field.COLUMN_NAME == fileField %=>
            { label: "<=$ field.COLUMN_COMMENT $=>", model: "file", md: 3, tag: "v-file-input", rules: "validationRules.requireRules" },
            //===//<=%- else %=>
  ...
  common: { 
    ...
    methods: {
      ...
      async prepareUpdateFormData(funObj) {
        ...
        //===// item.file = { name: (item.<=$ fileField $=> || '').split('/').pop() };
        this.updateItem = _.cloneDeep(item);
        this.updateItemOrigin = _.cloneDeep(item);
      },
      async filePreview(item) {
        this.updateItem = item;
        //===// if (!item.<=$ fileField $=>) return;
        //===// const { <=$ fileField $=> } = item;
        //===// const filename = <=$ fileField $=>.split('/').pop();
        this.previewFileType = null;
        if (/\.(gif|jpg|jpeg|png|GIF|JPG|PNG)$/.test(filename)) {
          this.previewFileType = 'img';
          //===// this.previewFileUrl = this.previewPrefix + item.<=$ fileField $=>;
        }
        if (/\.(pdf|PDF)$/.test(filename)) {
          this.previewFileType = 'pdf';
          //===// this.previewFileUrl = encodeURIComponent(this.previewPrefix + item.<=$ fileField $=>);
        }
      ...
      async downloadFileByStream({ item }) {
        //===// const downloadPath = item.<=$ fileField $=>
        //===// const filename = item.<=$ fileField $=>.split('/').pop();
        ...
        for (const file of fileList) {
          if (file && file.size) {
            ...
            if (result.data.status === 'success') {
              ...
              this[key]['<=$ fileField $=>'] = result.data.appData.resultData.downloadPath
