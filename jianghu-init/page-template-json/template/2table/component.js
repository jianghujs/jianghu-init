/* eslint-disable */
const content = {
  pageType: "<=$ pageType $=>", pageId: "<=$ pageId $=>", table: "<=$ table $=>", pageName: "<=$ pageName $=>", template: "jhTemplateV4", version: 'v2', componentPath: '<=$ componentName $=>',
  resourceList: [
    {
  ...
  pageContent: [
    {
      ...
      headActionList: [
        ...
        { tag: 'v-btn', value: '分配<=$ nameB $=>', attrs: { color: 'success', class: 'mr-2', '@click': 'doUiAction("viewAllot")' }, quickAttrs: ['small'] },
        { tag: 'v-spacer' },
        /*html*/`
  ...
  common: { 
    ...
    computed: {
      ...
      tableDataAllot() {
        //===// return this.tableData.filter(e => e.<=$ primaryFieldA $=> != this.<=$ primaryFieldA $=>);
      },
    },
    async created() {
    ...
    methods: {
      ...
      async confirmAllotItemDialog() {
        if (await window.confirmDialog({ title: "确定将班级分配给此<=$ nameB $=>吗", content: "确定分配吗？" }) === false) {
        ...
        await window.jianghuAxios({
          data: {
            appData: {
              pageId: '<=$ pageId $=>',
              actionId: '<=$ actionIdPrefix $=>-updateItem',
              //===// actionData: { <=$ primaryFieldA $=>: this.<=$ primaryFieldA $=> },
      ...
      tableItemSelected({ item, value }) {
        if (value) {
          ...
          //===// this.allotTableSelected = _.reject(this.allotTableSelected, ['<=$ primaryFieldA $=>', item.<=$ primaryFieldA $=>]);
        }
      },
