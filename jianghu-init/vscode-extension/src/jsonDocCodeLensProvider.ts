...
export class JsonDocCodeLensProvider implements vscode.CodeLensProvider {
  ...
  // 文档URL映射
  private static docUrlMap: Record<string, string> = {
    // 关键属性文档URL
  ...
  // 默认文档URL
  private static defaultDocUrl = 'https://www.openjianghu.org/doc/page/article/11105';
   ...
   */
  private findDocumentation(propertyName: string): {url?: string, filePath?: string} {
    // 首先检查是否有URL映射
    ...
    // 如果没有URL映射，查找本地文档
    const extensionPath = vscode.extensions.getExtension('jianghujs.jianghu-init-vscode')?.extensionPath;
    if (!extensionPath) {
      return {};
    }
 ...
 * @param propertyName 属性名称
 */
export async function showPropertyDoc(docInfo: {url?: string, filePath?: string}, propertyName: string): Promise<void> {
  try {
    let content = '';
    let isHtml = false;
    let isIframe = false;
