...
exports.activateV6ConfigHover = exports.V6ConfigHoverProvider = void 0;
...
const V6_PATH_DOCS = {
    ...
    'pageContent|component': {
        type: 'string',
        description: '**pageContent**（页面主体）中的 `component`，仅允许布局与主体业务组件（与 v6 schema `pageNode` 枚举一致）：\n' +
    ...
    'actionContent|component': {
        type: 'string',
        description: '**actionContent**（操作区 / 覆盖层）中的 `component`，一般为抽屉或底部叠层：\n' +
            '- `"CreateDrawer"` — 新增表单抽屉\n' +
            '- `"UpdateDrawer"` — 编辑表单抽屉\n' +
            '- `"Drawer"` — 通用抽屉（自定义主体）\n' +
            '- **`FormDrawer`** — 配置式表单抽屉\n' +
    ...
    component: {
        type: 'string',
        description: '`component` 的合法取值取决于所在数组：写在 **pageContent** 与写在 **actionContent** 中的枚举不同。将光标悬停在对应区域内的 `component:` 键上可查看完整列表。',
    },
    ...
    'PageHeader|keyword': {
        type: 'string',
        description: 'keyword 搜索词。**字符串**时表示 Vue 变量名（生成 `:keyword.sync="该名"`）；默认绑定页面 `keyword`。\n' +
            '与 **`jh-mobile-search-sheet`（SearchSheet）** 约定一致。',
    },
    'PageHeader|keywordFieldList': {
        description: 'keyword 已选列。**数组**为静态配置；**字符串**为变量名（生成 `:keyword-field-list.sync`）。默认绑定页面 `keywordFieldList`。\n' +
    ...
    'MobileSearch|searchFieldList': {
        description: '**`jh-mobile-search-sheet`** 的 `searchFieldList`（与 PageHeader 一致，可有 `type: "keyword"`）。省略时解析阶段可继承 PageHeader / `list.search.fields`。',
    },
    ...
    'MobileFilterBtn|labelBind': {
        type: 'string',
        description: '**推荐**：绑定主文案的 Vue 表达式（整段原样写入 `:label="…"`），如 `pageTitle`、`row.modeName`。与 **`label` 二选一**（同时写时本键优先并忽略 `label`）。',
    },
    ...
    'MobileFilterBtn|showActive': {
        type: 'boolean',
        description: '`true` 且第二行有内容（**`activeDisplayBind`** / **`activeDisplay`** 或 **`children`** 的 **`v-slot:active-display`**）时显示双行摘要，否则圆角按钮。',
    },
    'MobileFilterBtn|activeDisplayBind': {
        type: 'string',
        description: '**推荐（单行）**：摘要第二行 Vue 表达式 → `:active-display="…"`。多段 + HTML 徽章等请用 **`children`** + **`v-slot:active-display`**。',
    },
    'MobileFilterBtn|activeDisplay': {
        description: '摘要第二行**静态**；动态单行用 **`activeDisplayBind`**；富文本用 **`children` 插槽**。',
    },
    'MobileFilterBtn|children': {
        description: '**`string[]`**：`<template v-slot:active-display>…</template>`。与 Table/List 的 slot children 写法相同。',
    },
    'MobileFilterBtn|iconBind': {
        type: 'string',
        description: '**推荐**：图标名为 Vue 表达式（`:icon="…"`）。**静态**图标名（含 **`filter-2`**）请写 **`icon`**，勿写 `iconBind`，否则表达式非法。',
    },
    ...
    'Sheet|actionList': {
        description: '**内容区**图标网格（与 orderList / children 三选一）。项 `{ value, icon?, color? }`；`cols` 控制列数。\n'
            + '点击 `@action` → `doUiAction(整项对象)`。**不支持** disabled / visible / intent。',
    },
    'Sheet|headActionList': {
        description: '**标题栏**按钮 → `jh-mobile-actions`（与 orderList 标题栏重置/确认互斥）。`@head-action` → `doUiAction`；或 `v-slot:head-actions` 覆盖。',
    },
    ...
    'SearchSheet|searchFieldList': {
        description: '与 **PageHeader `searchFieldList`** 完全一致。**数组**静态字段；**字符串**为变量名 → `:search-field-list`（勿把整段 `[...]` 写成字符串误绑）。',
    },
    'SearchSheet|keywordHeaders': {
        description: 'keyword 可选列。**数组**或 **字符串**变量名 → `:keyword-headers`；可与 Table `headers` 同源。',
    },
    'SearchSheet|keyword': {
        type: 'string',
    ...
    'Drawer|fieldList': {
        description: '表单字段配置数组（无 `tabList` 时写在 Drawer 根上；有 `tabList` 时每个 tab 各自一份）。\n\n'
    ...
    'fieldList|autoId': {
        type: 'object',
        description: '自动 ID 生成配置 `{ type, prefix, start }`。\n'
            + '- `type`: 生成器类型（枚举）\n'
    ...
    'searchFieldList|options': {
        description: 'select 下拉：静态数组，或状态路径字符串 "constantObj.xxx"。\n' +
    ...
    'filterList|options': {
        description: 'select 下拉：静态数组，或状态路径字符串；PC Table 客户端 filter 用（语法同 searchFieldList.options）',
    },
...
// ─────────────────────────────────────────────────────────────────────────────
function isV6ConfigFile(document) {
    return (0, configVersionDetect_1.isV6ConfigDocument)(document);
}
// ─────────────────────────────────────────────────────────────────────────────
// 获取光标所在行的缩进量（空格数）
// ─────────────────────────────────────────────────────────────────────────────
...
function getV6UiTreeSection(ancestors) {
...
function lookupDoc(word, ancestors, componentType) {
    ...
    if (word === 'component' && nearestList !== 'fieldList') {
        ...
        if (section)
            candidates.push(`${section}|component`);
    }
    // 1. 完整祖先路径（最多 3 层）
    ...
    for (const c of candidates) {
        if (seen.has(c))
            continue;
        seen.add(c);
        if (c in V6_PATH_DOCS)
            return V6_PATH_DOCS[c];
    }
    return null;
}
...
exports.V6ConfigHoverProvider = V6ConfigHoverProvider;
// ─────────────────────────────────────────────────────────────────────────────
// 注册入口
// ─────────────────────────────────────────────────────────────────────────────
function activateV6ConfigHover(context) {
...
exports.activateV6ConfigHover = activateV6ConfigHover;
//# sourceMappingURL=v6ConfigHoverProvider.js.map