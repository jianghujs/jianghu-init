...
exports.V7_PATH_DOCS = {
    ...
    'form|pcAttrs': {
        type: 'object',
        description: '**PC 端 attrs 覆写**（target=pc）。**直接是 attrs 对象**，merge 于 `form.attrs`：`{ rows: 8 }`。',
    },
    'form|mobileAttrs': {
        type: 'object',
        description: '**Mobile 端 attrs 覆写**（target=mobile）。**直接是 attrs 对象**，merge 于 `form.attrs`。',
    },
    views: {
    ...
    'create|mobileSheet': {
        description: '**FormSheet 专用**（mobile create）。控制内容区最大高度和填满/自然撑开模式（**默认 fill**）。\n'
    ...
    'update|mobileSheet': {
        description: '**FormSheet 专用**（mobile update）。使用 maxBodyHeight 控制可用高度（**默认 bodyHeightMode: fill**）；子键见 **`sheet|*`**。',
    ...
    'slots|create': {
        description: '新增表单插槽。使用 `pc/mobile.children`。\n'
            + '- 字段：`v-slot:field-{key}`\n'
            + '- 整 Tab：`v-slot:tab-{tabKey}` 或单表单 `v-slot:tab-default`',
    },
    'slots|update': {
        description: '编辑表单插槽。使用 `pc/mobile.children`；多 Tab 时使用 `{tabKey}.pc.children` 追加 template。\n'
    ...
    'slots|update|pc': {
        description: 'PC UpdateDrawer 子节点：`children: string[]`。\n'
    ...
    'Drawer|fieldList': {
        description: '单表单字段数组（无 tabList 时）。来自 **views.create.fieldList** / **views.update.fieldList** + **fields** 字典；也可手写覆写 blocks。\n\n'
            + '项形状见 **`fieldList|*`**；`type: section|divider|tip` 为分组占位。',
        example: '[{ key: "projectName", label: "项目名称", type: "text", required: true }]',
    },
    'Drawer|tabList': {
        description: '多 Tab 编辑（**views.update.tabList**）。每项：`{ key, title, fieldList, actionList?, cols?, ... }`。',
    },
    'Drawer|actionList': {
        description: '表单操作按钮（**views.create/update.actionList**）。Drawer 渲染在底部，FormSheet 渲染在标题栏。',
    },
    ...
    'SearchSheet|fieldList': {
        description: '搜索字段数组，来自 **views.list.search.fieldList** 展开；可含 `type: "keyword"`。',
    },
    ...
    'Sheet|actionList': {
        description: '**标题栏**按钮（与 FormSheet / Drawer 一致 → `jh-mobile-actions`）。\n'
    ...
    'MobileActions|actionList': {
        type: 'array',
        description: '顶栏按钮数组。项 `{ label, uiAction, color?, visibleWhen?, disabledWhen?, loadingWhen? }` → `@action` → `doUiAction`。',
    },
    // ── Box / Grid（布局容器）──────────────────────────────────────────────────
...
const collectChildren = (prefix) => {
    ...
    for (const path of Object.keys(exports.V7_PATH_DOCS)) {
        if (!path.startsWith(pfx))
            continue;
        if (exports.V7_PATH_DOCS[path].deprecated)
            continue;
        ...
        if (segs.length === 1) {
            g.doc = exports.V7_PATH_DOCS[path];
        }
        else {
            const nextKey = segs[1];
            if (!g.subKeys.has(nextKey)) {
                g.subKeys.set(nextKey, exports.V7_PATH_DOCS[`${pfx}${first}|${nextKey}`]);
            }
        }
    }
...
function scanInlineObjectAncestors(linePrefix) {
    const stack = [];
    let quote = '';
    let escaped = false;
    ...
    return stack.filter((key) => !!key).reverse();
}
exports.scanInlineObjectAncestors = scanInlineObjectAncestors;
...
function lookupDoc(word, ancestors, componentType) {
    ...
    for (const c of candidates) {
        if (seen.has(c))
            continue;
        seen.add(c);
        if (c in exports.V7_PATH_DOCS)
            return exports.V7_PATH_DOCS[c];
    }
    return null;
}
...
class V7ConfigHoverProvider {
    provideHover(document, position) {
        ...
        if (isParamHover && overrideCtx) {
            doc = lookupDoc(word, [overrideCtx.platform, 'override'], null) || exports.V7_PATH_DOCS[`override|${word}`] || null;
            if (word === 'blocks') {
            ...
            doc = exports.V7_PATH_DOCS[`blocks|${word}`] || lookupDoc(word, ['blocks'], null) || null;
        }
        else {
...
class V7ConfigSignatureHelpProvider {
    provideSignatureHelp(document, position) {
        ...
        if (!overrideCtx || !overrideCtx.paramRange.contains(position))
            return null;
        const viewsDoc = exports.V7_PATH_DOCS['override|views'];
        const blocksDoc = exports.V7_PATH_DOCS['override|blocks'];
...
class V7ConfigCompletionProvider {
    provideCompletionItems(document, position) {
        ...
        if (/\bblocks\.[a-zA-Z$_][a-zA-Z0-9$_]*$/.test(before) || /\bblocks\.$/.test(before)) {
            ...
            if (!/^\s*[a-zA-Z$_][a-zA-Z0-9$_]*$/.test(before) && !/^\s*$/.test(before))
                return null;
            const { ancestors, componentType } = scanAncestors(document, position);
