 ...
 */
module.exports = class InitPageManual extends CommandBase {
   ...
   * 生成 pageDoc
   */
  async renderPageDoc(dirPath, { pageId }) {
    const templatePath = `${path.join(__dirname, '../../')}page-template`;
    if (fs.existsSync(`${templatePath}/${dirPath}/manual.md`)) {
      let md = fs.readFileSync(`${templatePath}/${dirPath}/manual.md`).toString();
      if (md) {
        md = md.replace(/\{\{pageId}}/g, pageId.slice(0, 1).toUpperCase() + pageId.slice(1));
        if(!fs.existsSync('./app/view/pageDoc')) {
          fs.mkdirSync('./app/view/pageDoc');
        }
        const mdPath = './app/view/pageDoc/manual.md';
        if (fs.existsSync(mdPath)) {
          const overwrite = await this.readlineMethod(`文件 ${mdPath} 已经存在，是否覆盖?(y/N)`, 'n');
          if (overwrite !== 'y' && overwrite !== 'Y') {
            this.warning(`跳过 ${mdPath} 的生成`);
            return false;
          }
        }
