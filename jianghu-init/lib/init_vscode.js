...
module.exports = class CommandInitVSCode {
  constructor() {
    ...
    this.prebuiltVsixPath = path.join(__dirname, '..', 'vscode-extension', 'prebuilt', 'jianghu-init-vscode-0.0.6.vsix');
  }
  ...
  async run(cwd, argv) {
    ...
    try {
      if (installType === 'simple') {
        await this.simpleInstall(editor);
      } else {
        await this.fullInstall(editor);
      }
    } catch (error) {
  ...
  async simpleInstall(editor) {
    ...
    try {
      // 根据系统选择不同的安装命令
      if (process.platform === 'darwin') {
        // Mac系统尝试使用VSCode或Cursor安装
        if (editor === 'VSCode') {
          await this.executeCommand(`/Applications/Visual\\ Studio\\ Code.app/Contents/Resources/app/bin/code --install-extension "${this.prebuiltVsixPath}"`);
   ...
   * 完整安装 - 从源码构建
   */
  async fullInstall(editor) {
    console.log(chalk.blue('使用完整安装模式...'));
    await this.installExtension(editor);
  }
  ...
  async installExtension(editor) {
    ...
    try {
      ...
      try {
        if (process.platform === 'darwin') {
          // Mac系统尝试使用VSCode或Cursor卸载
          if (editor === 'VSCode') {
            await this.executeCommand(`/Applications/Visual\\ Studio\\ Code.app/Contents/Resources/app/bin/code --uninstall-extension jianghujs.jianghu-init-vscode`);
          } else if (editor === 'Cursor') {
            await this.executeCommand(`/Applications/Cursor.app/Contents/Resources/app/bin/code --uninstall-extension jianghujs.jianghu-init-vscode`);
          }
        } else {
          // Windows或Linux系统
      ...
      if (process.platform === 'darwin') {
        // Mac系统尝试使用VSCode或Cursor安装
        if (editor === 'VSCode') {
          await this.executeCommand(`/Applications/Visual\\ Studio\\ Code.app/Contents/Resources/app/bin/code --install-extension "${path.join(this.vscodeExtensionPath, vsixFile)}"`);
        } else if (editor === 'Cursor') {
          await this.executeCommand(`/Applications/Cursor.app/Contents/Resources/app/bin/code --install-extension "${path.join(this.vscodeExtensionPath, vsixFile)}"`);
        }
      } else {
        // Windows或Linux系统
