...
const InitPage = require('./uni_json/init_page');
const InitComponent = require('./uni_json/init_component');
const InitJson = require('./uni_json/init_json');
...
module.exports = class InitByJsonCommand extends CommandBase {
  ...
  async promptConfig() {
    ...
    if ([ 'page' ].includes(type)) {
      generateFileDir = './init-json/page';
    } else if ([ 'component' ].includes(type)) {
      generateFileDir = './init-json/component';
    } else {
    ...
    if (fs.existsSync(generateFileDir)) {
      ...
      if (!fileItem) {
        // 获取文件夹 generateFileDir 下所有的文件, value 为文件夹 + 文件名
        const fileList = await this.getAllFiles(generateFileDir);
        const res = await inquirer.prompt({
          name: 'fileItem',
          type: 'list',
          choices: fileList,
  ...
  async getAllFiles(dirPath, fileList = []) {
    ...
    files.forEach(file => {
      ...
      if (fileStats.isDirectory()) {
        // 如果是目录，则递归调用 getAllFiles
        this.getAllFiles(filePath, fileList);
      } else {
        // 如果是文件，则添加到 fileList 数组中
        fileList.push({ value: filePath.replace('init-json/page/', ''), name: filePath.replace('init-json/page/', '') });
      }
    });
  ...
  async enableDevMode(dev) {
    ...
    if (!fs.existsSync('./init-json')) {
      fs.mkdirSync('./init-json', { recursive: true });
    }

    await lockfile.lock(lockFilePath);

    // 默认渲染 ./init-json 内的递归所有文件
    const fileList = await this.findJsFiles('./init-json');
    ...
    for (const file of fileList) {
      ...
      configFileList.push({ pageType: fileObj.pageType, pageId: fileObj.pageId, componentPath: fileObj.componentPath, file: file.replace('init-json/', '') });
      await this.renderContent(fileObj, file.replace('init-json/', ''));
    }
    this.success(`共有 ${fileList.length} 个配置文件，加载完成`);
    this.checkFileRepeat(configFileList);

    // 监控文件变化
    const watcher = chokidar.watch('./init-json', {
      persistent: true,
      ignoreInitial: true,
    ...
    watcher.on('all', async (event, pathStr) => {
      if (event === 'change') {
        this.info(`File ${pathStr.replace('init-json', '')} change ${dayjs().format('HH:mm:ss')}`);
        let fileObj = {};
        try {
          // eslint-disable-next-line no-eval
        ...
        await this.renderContent(fileObj, pathStr.replace('init-json/', ''));
    ...
    console.log('Watching ./init-json for changes...');
  ...
  checkJhJsonExists(fileObj, warning) {
    // todo warning.push('请在 includeList 中添加 jh-json-editor 资源引入\n    ' + warn.join('\n    '));
  }
