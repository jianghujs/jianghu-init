...
) ENGINE = InnoDB AUTO_INCREMENT = 1977 COMMENT = '数据历史表';
...
) ENGINE = InnoDB AUTO_INCREMENT = 15 COMMENT = '文件表; 软删除未启用;';
...
where
  (
  `{{dbPrefix}}data_repository`.`{{dbPrefix}}user_app_management___user_app`.`appId` = '{{name}}'
  );
