...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `view01_category` AS select `__name__`.`category`.`id` AS `id`,ifnull(`article_category`.`articleCount`,0) AS `articleCount`,`__name__`.`category`.`categoryId` AS `categoryId`,`__name__`.`category`.`categoryName` AS `categoryName`,`__name__`.`category`.`categoryArticleIgnoreTiltle` AS `categoryArticleIgnoreTiltle`,`__name__`.`category`.`categoryGroup` AS `categoryGroup`,`__name__`.`category`.`categoryGroupSort` AS `categoryGroupSort`,concat('<',`__name__`.`category`.`categoryGroup`,`__name__`.`category`.`categoryGroupSort`,'>') AS `categoryGroupConcat`,`__name__`.`category`.`categoryIntro` AS `categoryIntro`,`__name__`.`category`.`categoryPublishStatus` AS `categoryPublishStatus`,`__name__`.`category`.`categoryCreateTime` AS `categoryCreateTime`,`__name__`.`category`.`categoryCreateUserId` AS `categoryCreateUserId`,`__name__`.`category`.`categoryCreateUsername` AS `categoryCreateUsername`,`__name__`.`category`.`categoryUpdateTime` AS `categoryUpdateTime`,`__name__`.`category`.`categoryUpdateUserId` AS `categoryUpdateUserId`,`__name__`.`category`.`categoryUpdateUsername` AS `categoryUpdateUsername`,`__name__`.`category`.`operation` AS `operation`,`__name__`.`category`.`operationByUserId` AS `operationByUserId`,`__name__`.`category`.`operationByUser` AS `operationByUser`,`__name__`.`category`.`operationAt` AS `operationAt` from (`__name__`.`category` left join (select `__name__`.`article`.`categoryId` AS `categoryId`,count(`__name__`.`article`.`categoryId`) AS `articleCount` from `__name__`.`article` group by `__name__`.`article`.`categoryId`) `article_category` on((`__name__`.`category`.`categoryId` = `article_category`.`categoryId`)));
